...
namespace mirp{		
	...
	struct Model{
		...
		void improvementPhaseVND_vessels(IloEnv& env, Instance inst, const double& timePerIter, const double& gap, double& obj1stPhase, Timer<chrono::milliseconds>& timer_cplex,float& opt_time,const double& timeLimit, float& elapsed_time, unsigned int& stopsByGap,unsigned int& stopsByTime, bool& isFeasibleRF);
		...
		void improvementPhase_typePortsLS(IloEnv env, Instance inst, const double& timePerIter, const int& gap,Timer<chrono::milliseconds>& timer_cplex, float& opt_time, const double& timeLimit, float& elapsed_time, double& incumbent, unsigned int& stopsByGap,unsigned int& stopsByTime,bool& isFeasibleRF);
		...
		bool isFeasible(IloEnv &env, Instance inst);
		bool removeBetas(IloEnv &env, Instance inst);
	};
}
